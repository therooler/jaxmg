                               ...
                               ffi::Result<ffi::AnyBuffer> out, ffi::Result<ffi::Buffer<ffi::S32>> status)
        {
            ...
            if (currentDevice == 0)
            {
                ...
                for (int peer = 1; peer < nbGpus; ++peer)
                {
                    ...
                    void *opened = nullptr;
                    std::printf("peer %d\n", peer);
                    cudaError_t oe = cudaIpcOpenMemHandle(&opened, *(cudaIpcMemHandle_t *)&shm->memHandle[peer], cudaIpcMemLazyEnablePeerAccess);
                    if (oe != cudaSuccess)
                    {
                        std::fprintf(stderr, "device %d: cudaIpcOpenMemHandle(peer=%d) failed: %d (%s)\n",
                                     currentDevice, peer, static_cast<int>(oe), cudaGetErrorString(oe));
                        return ffi::Error::Internal(absl::StrFormat("cudaIpcOpenMemHandle failed for peer %d: %s",
                                                                    peer, cudaGetErrorString(oe)));
                    }
                    ...
                    JAX_FFI_RETURN_IF_GPU_ERROR(gpuMemcpy(
                        ev_print.data(), opened, sizeof(data_type) * N * batch_a, gpuMemcpyDeviceToHost));
                    CUDA_CHECK_OR_RETURN(cudaDeviceSynchronize());
                    // sync_point.arrive_and_wait();
                    print_matrix(N, batch_a, ev_print.data(), N);
                    // shmA[peer] = static_cast<data_type *>(opened);
                    CUDA_CHECK_OR_RETURN(cudaIpcCloseMemHandle(opened));
                    // keep opened handle until you're done; close with cudaIpcCloseMemHandle when finished
                }
            }
            CUDA_CHECK_OR_RETURN(cudaDeviceSynchronize());
            // sync_point.arrive_and_wait();
